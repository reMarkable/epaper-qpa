...
//
// This mapping is the same as the US mapping, but with a few specific keys changed,
// to match up with the RM's custom layout.
...
const EpaperEvdevKeyboardMap::Mapping s_keymap_us_rm[] = {
    ...
    // START RM CHANGE:
    // for KEY_GRAVE (41), we want ´ (ACCUTE ACCENT, 0x00b4) instead of ~ (TILDE, 0x007e) when shifted.
    { 41, 0x0060, 0x00000060, 0x00, 0x00, 0x0000 },
    { 41, 0x00b4, 0x000000b4, 0x01, 0x00, 0x0000 },
    { 41, 0x0060, 0x01001250, 0x02, 0x01, 0x0000 },
    { 41, 0x00b4, 0x000000b4, 0x03, 0x01, 0x0000 }, // note: on standard US, this was mapped to Qt::Key_Dead_Tilde before... that might mean that the compose mappings need to change.
    // END RM CHANGE:

    { 42, 0xffff, 0x01000020, 0x00, 0x04, 0x0001 },

    // START RM CHANGE:
    // we don't have KEY_BACKSPACE. Instead of \|, we want ~¨.
    { 43, 0x007e, 0x0000007e, 0x00, 0x00, 0x0000 },
    { 43, 0x00a8, 0x000000a8, 0x01, 0x00, 0x0000 },
    { 43, 0x007e, 0x0400007e, 0x04, 0x00, 0x0000 },
    // END RM CHANGE:
